...
<div class="master">
    <div >
        <div style="margin-left:10px">
            <h5 style="text-align:left;font-size:18px;font-weight:bolder">质控 自研项目测试需求列表</h5>
        </div>
        <hr>
   ...
   <div style="display:flex">
    ...
    <el-table-column label="处理人" min-width="100">
         <template v-slot="scope">
            <h6 style="font-size:13px;margin:8px 0px 8px 0px;font-weight: normal">{{scope.row.owner}}</h6>
    ...
    <el-table-column :label="'是否延期'+delay_state" min-width="130">
     ...
     <el-table-column label="状态" min-width="150">
         <template v-slot="scope">
            <h6 style="font-size:13px;border-radius: 7px;margin:8px 0px 8px 0px;font-weight: normal;" :class="(scope.row.status_name=='实现中')?'ing':(scope.row.status_name=='已关闭')?'closed':(scope.row.status_name=='规划中')?'plan':(scope.row.status_name=='已拒绝')?'closed':'reselve'">{{scope.row.status_name}}</h6>
        </template>
    </el-table-column>
     ...
     <el-table-column label="修改难度" min-width="150">
        <template v-slot="scope">
                 ...
                 <el-select v-model="scope.row.rankvalue" :placeholder="'难易度：'+scope.row.difficulty" style="margin-bottom:2px;margin:8px 0px 8px 0px;font-weight: normal;" @change="rankchange(scope.row)">
                    <el-option
...
export default {
    ...
    methods:{
        apiresponse(){
            hpaxios("/api/v1/DataTable/",{type:"issue",data_type:"zy",projects:["质量管理跟进项目需求"],iteration:"所有迭代",page:this.apipage,page_size:this.apipage_size,search:this.input, fields:["id","name","priority","begin","owner","workspace_id","due","url","status_name","difficulty"]})
                  .then(res => {
        ...
        delay(row){
            if(row == "未延期"){
                ...
                return " background-color: #f85e5e;padding:5px;padding-top:2px;padding-bottom:2px;padding-right:0px;width:55px;color:#f0f0f0"
            }
        },
         delay_sort(column){
       ...
       cellStyle({columnIndex}){
                ...
                // "border-right:solid 1px #7d7d7d;border-left:solid 1px #7d7d7d;;border-bottom:solid 1px #7d7d7d"
        },
        rowstyle({rowIndex}){
           for (var i = 0;i < 12;i++){
                if (rowIndex%2==0){
       ...
       run(){
        ...
        rankchange(row){
            hpaxios("/api/v1/SetIssueDifficulty/",{id:row.id,difficulty:row.rankvalue,cache_check:{type:"issue",data_type:"zy",projects:["质量管理跟进项目需求"],iteration:"所有迭代"}})
                  .then(res => {
                      console.log(res);
