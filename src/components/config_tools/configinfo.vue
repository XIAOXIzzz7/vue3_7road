...
<div class="master">
    ...
    <div style="display:flex;justify-content:space-between;padding-left:10px">
        <div style="display:flex;margin-top:20px">
            <el-button @click="add()"  v-if="$route.params.id =='member' || $route.params.id =='project'" style="padding:7px;height:30px" type="primary" plain ><h6 style="margin:0px;font-size:12px">添加{{cnname}}</h6></el-button>
            <el-button @click="membercount()" v-if="$route.params.id =='member'"  style="padding:7px;height:30px" type="primary" plain ><h6 style="margin:0px;font-size:12px">人员数量</h6></el-button>
            <el-button @click="admin()" v-if="$route.params.id =='member'" style="padding:7px;height:30px;min-width:64px" type="primary" plain ><h6 style="margin:0px;font-size:12px">管理员</h6></el-button>
        </div>
        <div style="text-align:right;margin-top:15px;margin-bottom:10px">
            <el-input v-if="$route.params.id =='group'" style="width:130px;margin-right:50px;text-align:right" v-model="input" @input="inputchange()"  placeholder="搜索人员"   prefix-icon="el-icon-search"> </el-input>
    ...
    <div style="padding-left:10px;display:flex;flex-direction:column">
        <div style="height:800px; min-height:800px">
        <el-table  :data="tableData">
            <el-table-column  label="id" min-width="10">
                <template v-slot="scope">
                    <h6 style="text-align:left;width:70px;padding:5px;font-size:14px;margin:12px 0px 12px 0px;font-weight: normal">{{scope.row.id}}</h6>
                </template>
            </el-table-column>
             <el-table-column   v-if="$route.params.id =='project'" label="项目归属" min-width="10">
                <template v-slot="scope">
                    <h6  style="text-align:left;width:70px;padding:5px;font-size:14px;margin:12px 0px 12px 0px;font-weight: normal">{{scope.row.fa}}</h6>
                </template>
            </el-table-column>
             <el-table-column  :label="cnname" min-width="10">
                <template v-slot="scope">
                    <h6  style="text-align:left;width:200px;padding:5px;font-size:14px;margin:12px 0px 12px 0px;font-weight: normal">{{scope.row.name}}</h6>
                </template>
            </el-table-column>
            <el-table-column   v-if="$route.params.id =='member'" label="部门" min-width="10">
                <template v-slot="scope">
                    <h6  style="text-align:left;width:70px;padding:5px;font-size:14px;margin:12px 0px 12px 0px;font-weight: normal">{{scope.row.group}}</h6>
                </template>
            </el-table-column>
            <el-table-column v-if="$route.params.id =='group'" label="部门" min-width="10">
                <template v-slot="scope">
                    <h6  style="text-align:left;width:200px;padding:5px;font-size:14px;margin:12px 0px 12px 0px;font-weight: normal">{{scope.row.project}}</h6>
                </template>
            </el-table-column>
            <el-table-column v-if="$route.params.id =='group'" label="组别" min-width="10">
                <template v-slot="scope">
                    <h6  style="text-align:left;width:200px;padding:5px;font-size:14px;margin:12px 0px 12px 0px;font-weight: normal">{{scope.row.group}}</h6>
                </template>
            </el-table-column>
            <el-table-column v-if="$route.params.id =='group'" label="权限" min-width="10">
                <template v-slot="scope">
                    <h6  style="text-align:left;width:200px;padding:5px;font-size:14px;margin:12px 0px 12px 0px;font-weight: normal">{{scope.row.permission}}</h6>
                </template>
            </el-table-column>
            <!-- <el-table-column   v-if="$route.params.id =='member'" label="部门" min-width="10">
                <template v-slot="scope">
             ...
             <el-table-column   v-if="$route.params.id =='member'" label="人员情况" min-width="10">
                <template v-slot="scope">
                    <h6  style="text-align:left;width:70px;padding:5px;font-size:14px;margin:12px 0px 12px 0px;font-weight: normal">{{scope.row.status}}</h6>
                </template>
            </el-table-column>
             <el-table-column   v-if="$route.params.id =='project'" label="项目ID" min-width="10">
                <template v-slot="scope">
                    <h6  style="text-align:left;width:200px;padding:5px;font-size:14px;margin:12px 0px 12px 0px;font-weight: normal">{{scope.row.project_id}}</h6>
                </template>
            </el-table-column>
             <el-table-column label="操作" min-width="10">
                 <template v-slot="scope">
                  <el-popover
                    ...
                    >
                    <div><h6 style="font-size:13px;font-weight:bold;color:C0C0C015;margin:12px 0px 12px 0px;font-weight: normal">{{cnname}}操作</h6></div>
                    <div style="display:flex;flex-direction:column">
                        <el-button v-if="$route.params.id =='group'" @click="permission(scope.row)" size="small" style="border:none;padding:0px;width:100px;margin:0px;font-weight: normal"><h6 style="font-size:13px;margin:0px;font-weight: normal">编辑{{cnname}}</h6></el-button>
                        <el-button v-if="$route.params.id =='member' || $route.params.id =='project'" @click="edit(scope.row)" size="small" style="border:none;padding:0px;width:100px;margin:0px;font-weight: normal"><h6 style="font-size:13px;margin:0px;font-weight: normal">编辑{{cnname}}</h6></el-button>
                        <el-button v-if="$route.params.id =='member' || $route.params.id =='project'" @click="del (scope.row)" size="small" style="border:none;padding:0px;width:100px;margin:0px;margin:0px;font-weight: normal"><h6 style="font-size:13px;margin:0px;font-weight: normal;margin:0px;font-weight: normal">移除{{cnname}}</h6></el-button>
                    </div>
                        <template #reference>
         ...
         <div class="block" style="margin:0px 20px 0px 0px">
            ...
            <el-dialog
             ...
             <el-dialog
                ...
                width="20%"
                >
                <div><h6><svg t="1636622032894" class="icon" viewBox="0 0 1024 1024" version="1.1" xmlns="http://www.w3.org/2000/svg" p-id="3981" width="25" height="25"><path d="M1001.661867 796.544c48.896 84.906667 7.68 157.013333-87.552 157.013333H110.781867c-97.834667 0-139.050667-69.504-90.112-157.013333l401.664-666.88c48.896-87.552 128.725333-87.552 177.664 0l401.664 666.88zM479.165867 296.533333v341.333334a32 32 0 1 0 64 0v-341.333334a32 32 0 1 0-64 0z m0 469.333334v42.666666a32 32 0 1 0 64 0v-42.666666a32 32 0 1 0-64 0z" fill="#FAAD14" p-id="3982"></path></svg>&nbsp;&nbsp;&nbsp;&nbsp;修改人员所权限&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</h6></div>
                <hr style="color:#C0C0C0">
                <div  style="margin-top:50px;display:flex">
                    <h6 style="text-align:left;margin-left:50px;font-size:14px;margin-bottom:0px;margin-top:7px;width:100px">组别：</h6> 
                    <div style="margin-left:0px;width:210px"><el-input v-model="groupname" size="small" placeholder="部门名称"></el-input></div>
                </div>
                <div  v-if="$route.params.id =='group'" style="margin-top:10px;display:flex">
                    <h6 style="text-align:left;margin-left:50px;font-size:14px;margin-bottom:0px;margin-top:7px;width:100px">权限：</h6> 
                    <div style="margin-left:50px">
                        <div style="width:200px">
                            <el-select v-model="perchoicevalue"  size="small" placeholder="请选择">
                            <el-option
                            v-for="item in perchoice"
                            :key="item.value"
                            :label="item.label"
                            :value="item.value"
                            >
             ...
             <el-dialog
                ...
                <div style="margin-left:20px">
                    <div style="text-align:left">
                        <el-button @click="addadmin()"  style="padding:7px;height:30px;margin:0px;font-weight: normal" type="primary" plain ><h6 style="margin:0px;font-size:12px;margin:0px;font-weight: normal">添加管理员</h6></el-button>
                    </div>
    ...
    methods:{
        mem(){
            ...
            params:{
                ...
                this.currentPage1 = 1
                this.tableData_all = lis
                this.list_total = lis.length
                this.list_info(this.currentPage1)
            })
        },
        list_info(nums){
            var dp=[0]
            var co = Math.ceil(this.tableData_all.length/10)
            for (var i=0;i<co;i++){
                dp.push(0)
            }
            dp[0]=0
            ...
            for(var i=start;i<count;i++){
                if (this.tableData_all[i]){
                    bug.push(this.tableData_all[i])
                }
            }
            this.tableData = bug
            console.log();
        ...
        inputchange(){
            if (this.input){
                var lis=[]
                var data = this.tableData_all
                for (var item in data){
                    if (data[item]["name"].indexOf(this.input) != -1 || data[item]["group"].indexOf(this.input) != -1) {
                        lis.push(data[item])
                    }
                }
                    this.currentPage1 = 1
        ...
        permission(row){
            ...
            if (row.permission!= ""){
                ...
                //     var nums = 0
                // }
                this.perchoicevalue = row.permission
            }
        ...
        permission1yes(){
            if(this.perchoicevalue == "admin" || this.perchoicevalue == 1){
                    var nums = "1"
                }
                else{
                    var nums = "0"
                }
            hpaxios("/api/v1/SetAuth/",{user_name:this.group2.user_name,group:this.groupname,project:this.group2.project,permission:nums})
                        .then(res => {
